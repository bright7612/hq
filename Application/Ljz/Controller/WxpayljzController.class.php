...
class WxpayljzController extends Controller
{
    /** 链接wechat接口 */
    public function index()
    {
        ...
        $moeny = 0.1;
        $tal_fee = $moeny * 100;
   //    echo I('post.rdurl'); exit;
        if(I('post.rdurl')){
            session("rdurl",I('post.rdurl'));
        }
        $rdurl = session("rdurl");
        ...
        $input->SetBody("党费在线支付");
        $input->SetAttach("党费在线支付");
        $input->SetOut_trade_no(\WxPayConfig::MCHID . date("YmdHis"));
        $input->SetTotal_fee($tal_fee);
        $input->SetTime_start(date("YmdHis"));
        $input->SetTime_expire(date("YmdHis", time() + 600));
        $input->SetGoods_tag("党费在线支付");
        $input->SetNotify_url("http://care.wiseljz.com/home/Wxpayljz/notify.html");
...
//        exit;
        ...
        $this->assign('rdurl',$rdurl);
        $ljz = session('ljzpost');
        $this->assign('ljzid',$ljz["id"]);
        $this->display();
    }
...
//				scoreLogAdd($searchmember['id'],$_POST['total_fee'],'充值获得'.$_POST['total_fee'].'贝',$addlog,'充值获得贝','inc');
            ...
            $post = session("ljzpost");
            $datapay['id'] = $post['id'];
            $datapay['state'] = 1;
            $pay = M("ljz.dues_pay","ljz_");
            $pay->save($datapay);
            $dtest['info'] = (M()->getLastSql()).$post['id'];
            M('test')->add($dtest);
