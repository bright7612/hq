...
//                $this->setSearchPostUrl('');
    ...
    public function key($name, $title, $type, $opt = null, $width = '550px')
    {
     ...
     * @auth 陈一枭
     */
    public function keyHtml($name, $title, $width = '550px')
    {
    ...
    public function keyState($name = 'state', $title = '状态')
    {
        $map = array(-1 => L('_DELETE_'), 0 => L('未审核'), 1 => L('审核通过'), 2 => L('未通过'), 3 => L('用户已取消'));
        return $this->key($name, $title, 'status', $map);
    }
    ...
    public function keyImage($name, $title,$opt=array())
    {
        return $this->key($name, $title, 'image',$opt);
    }
    ...
    public function keyDoAction($getUrl, $text, $title = '操作',$opt=array())
    {
        //获取默认getUrl函数
        if (is_string($getUrl)) {
            $getUrl = $this->createDefaultGetUrlFunction($getUrl);
        ...
        //在DOACTIONS中增加action
        $doActionKey['opt']['actions'][] = array('text' => $text, 'get_url' => $getUrl,"opt"=>$opt);
        return $this;
    }
    ...
    public function display($solist = '')
    {
        ...
        $this->convertKey('image', 'html', function ($value, $key, $item) {
            ...
            if (intval($value)) {//value是图片id
                ...
                $sc_src = $value;
                $html = "<div class='popup-gallery'><a title=\"" . L('_VIEW_BIGGER_') . "\" href=\"$sc_src\"><img src=\"$sc_src\"/  $content style=\"border-radius:100%;\"  ></a></div>";
            }
            return $html;
        });
        ...
        $this->convertKey('doaction', 'html', function ($value, $key, $item) {
            ...
            foreach ($actions as $action) {
                if(isset($item["_hide"]) && in_array($action['text'],$item["_hide"])){
                    continue;
                }
    ...
    public function doSetState($model, $ids, $state = 1)
    {
        $id = array_unique((array)$ids);
        $rs = M($model)->where(array('id' => array('in', $id)))->save(array('state' => $state));
        if ($rs === false) {
            $this->error(L('_ERROR_SETTING_') . L('_PERIOD_'));
        }
        $this->success(L('_SUCCESS_SETTING_'), $_SERVER['HTTP_REFERER']);
    ...
    public function doSetFields($model, $ids,$array)
    {
        $id = array_unique((array)$ids);
        $rs = M($model)->where(array('id' => array('in', $id)))->save($array);
        if ($rs === false) {
            $this->error(L('_ERROR_SETTING_') . L('_PERIOD_'));
        }
        $this->success(L('_SUCCESS_SETTING_'), $_SERVER['HTTP_REFERER']);
